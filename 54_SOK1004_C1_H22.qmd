...
df_tibble <- as_tibble(df$dataset) #lage en tibble 

df_tibble <- df_tibble %>%
  rename(var = ContentsCode) #rename alle variabel navn 
...
df_tibble <- df_tibble %>%  #endre tid variable fra chr -> int
...
df_tibble %>%
  ...
  labs(title = "BNP i løpende og faste priser",   # tittel
       caption = "Hentet fra SSB", # kilde
       x = "År", # x navn på x-akse
       y = "Brutto nasjonal produkt delt på 1000") #navn på y-akse

###
# prisene har økt i begge grafer fra 2000 til 2021.
# MEMOBNP(blå graf) viser Brutto national produkt i faste 2015 priser. 
# BNP(rød graf) viser BNP i løpende priser i tusenvis av kroner per person.
# det er ønskelig å skille mellom løpende og faste priser for å ta bort effekten av prisendring over tid.
# i 2015 ser vi at grafene krysser hverandre, dette er fordi vi tar utgangspunkt i 2015
```
...
###koden lager ett nytt datasett. det nye datasettet er endret. Navnet på kolonnen til BNP og MEMOPBNP istedenfor å bruke det som en beskrivelse av verdien. 
```
...
df_wide <- df_wide %>%
  ...
  mutate(DMEMOBNP = MEMOBNP - lag(MEMOBNP, n=1L)) #dette gjør vi for å få laget en tabell som viser faste priser fra 2015

df_wide <- df_wide %>%
  relocate(DBNP, .before = "MEMOBNP") #endre rekefølge i tabellen

df_prosmid <- df_wide%>%
  mutate(DMEMOBNP = MEMOBNP - LMEMOBNP) #lag en ny liste midlertidig for å ikke endre orginalen

df_prosent <- df_prosmid%>%
  mutate(Prosent_BNP = ((DBNP/BNP * 100))) %>%
  mutate(Prosent_MEMOBNP = ((DMEMOBNP/MEMOBNP * 100))) #lag en ny liste og endre til faste 2015 priser i prosent.
...
#lager ett nytt datasett, bytter tilbake til den orginale formen som er ett datasettet som er mer anvendelig i en graf.
...
df_long_prosent <- df_prosent %>%
  pivot_longer(c("Prosent_BNP", "Prosent_MEMOBNP"), names_to = "var", values_to = "verdi") #lag en liste som kun inneholder 2 kolonner som gjør det mulig å lage en graf med kun 2 linjer.
...
df_long_prosent %>%
  ...
  labs(title = "Prosentvis vekst i nominell og reel BNP per person fra 1970 til 
2021",
       caprion = "hentet fra SSB",
       x = "år",
       y = "verdi")
###
# Bankkrisen i Norge 1987–1992.
# finsanskrisnen i 2008.
# finansielle konsekvensene av covid-19 i mellom 2018-2020.
```
