...
Ved å bruke lave verdier i binwidth kommandoen blir søylene tynnere. Det er fordi du forteller R at du vil ha tynnerer søyler.

Eksperimenter med forskjellige verdier for `binwidth` og forklar hva som kjennetegner en god verdi.

**Oppgave 1**

En god binwidth kjennetegnes ved å vise mest mulig data, men som fortsatt er lett å lese av. Hvis du setter en for lav verdi kan det bli veldig mange søyler, å hvis du setter for høy verdi blir alle søylene slått sammen å det kan se ut som om noen søyler bare forvinner i histogrammet.
...
fouplot_1 <- fou %>% 
  ...
  geom_histogram(binwidth = 0.4, fill = "blue", alpha = 0.5)+
  theme_minimal()+
  facet_grid(yrker~gruppe)+
  labs(x = "Milliarder")
fouplot_1
```
...
fouplot_2 <- fou %>%
  ...
  theme_minimal()+
  facet_grid(yrker~gruppe)+
  labs(y = "antall observasjoner i sysselsettninger", x = "Milliarder")
fouplot_2
```
